...
- Tworzenie/nagrywanie evalsetów i metryki - moduł 7 (`ex_13_eval/`).
- Budowa samych agentów - `ex_14_text_to_sql`, `ex_16_report_system`.
...
z `ex_13_eval/test_config.json`. To most między "ręcznym" evalem a CI.

## Twoje zadanie
Najpierw nagraj test set systemu raportowego w `adk web ex_16_report_system` (zakładka
Eval), zapisz go do `ex_13_eval/report_system.evalset.json`, a potem odkomentuj
i uruchom `test_report_system.py`.

## Wskazówki (jeśli pracujesz bez agenta AI)
- `AgentEvaluator.evaluate(agent_module="ex_16_report_system", eval_dataset_file_path_or_dir=...)`.
- Evalset nagraj w `adk web ex_16_report_system` (zakładka Eval) albo dopisz ręcznie
  wg wzoru `ex_13_eval/sql_agent.evalset.json`.

## "Działa", gdy
`uv run pytest ex_17_tests` przechodzi (lub świadomie pomija test, którego evalsetu
