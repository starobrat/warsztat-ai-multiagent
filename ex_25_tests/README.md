...
- Tworzenie/nagrywanie evalsetów i metryki - moduł 7 (`ex_17_eval_uruchom/`).
...
z `ex_17_eval_uruchom/test_config.json`. To most między "ręcznym" evalem a CI.

## Twoje zadanie
Najpierw nagraj test set systemu raportowego w `adk web ex_22_report_writer` (zakładka
Eval), zapisz go do `ex_17_eval_uruchom/report_system.evalset.json`. Gdy plik już istnieje,
...
- Evalset nagraj w `adk web ex_22_report_writer` (zakładka Eval) albo dopisz ręcznie
  wg wzoru `ex_17_eval_uruchom/sql_agent.evalset.json`.

## "Działa", gdy
`uv run pytest ex_25_tests` przechodzi (lub świadomie pomija test, którego evalsetu
