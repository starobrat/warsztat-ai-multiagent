...
- Tworzenie nowych test case'ów - moduł 7 (`ex_06_evaluation/`).
- Testy automatyczne / pytest - moduł 12 (`ex_09_tests/`).
...
Uruchom:
```bash
uv run adk eval ex_07_sql_agent_tuning ex_06_evaluation/sql_agent.evalset.json \
...
- To samo pole `instruction` co w `ex_05_sql_agent` - tam masz wzór dobrej instrukcji.
