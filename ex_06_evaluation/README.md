...
uv run adk eval ex_05_sql_agent ex_06_evaluation/sql_agent.evalset.json \
    --config_file_path ex_06_evaluation/test_config.json --print_detailed_results
```

**2. Interfejs webowy** (moduł 7): `adk web ex_05_sql_agent` -> zakładka **Eval**.
...
**3. pytest** (moduł 12): `uv run pytest ex_09_tests`

## Twoje zadanie
Nagraj **2-3 własne case'y eval** dla `ex_05_sql_agent`: w `adk web ex_05_sql_agent`,
zakładka **Eval**, poprowadź rozmowę i zapisz ją jako eval case. Wykorzystaj pusty
szablon z `ex_06_evaluation/_szablony/`. Potem uruchom eval i sprawdź, czy agent je
