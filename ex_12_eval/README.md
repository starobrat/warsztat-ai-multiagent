...
(eval agenta SQL). W nowym układzie ex_12 ma ewaluować AGENTA NARZĘDZIOWEGO
(deterministyczne narzędzia = łatwiejsze ground-truth). Evalset SQL (sql_agent.evalset.json)
pasuje raczej do ex_13/ex_14. Do przepisania: evalset + framing pod agenta narzędziowego. -->
...
uv run adk eval ex_13_text_to_sql ex_12_eval/sql_agent.evalset.json \
    --config_file_path ex_12_eval/test_config.json --print_detailed_results
```

**2. Interfejs webowy** (moduł 7): `adk web ex_13_text_to_sql` -> zakładka **Eval**.
...
**3. pytest** (moduł 12): `uv run pytest ex_16_tests`

## Twoje zadanie
Nagraj **2-3 własne case'y eval** dla `ex_13_text_to_sql`: w `adk web ex_13_text_to_sql`,
zakładka **Eval**, poprowadź rozmowę i zapisz ją jako eval case. Wykorzystaj pusty
szablon z `ex_12_eval/_szablony/`. Potem uruchom eval i sprawdź, czy agent je
