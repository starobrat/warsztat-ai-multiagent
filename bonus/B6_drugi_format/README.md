# Bonus B6: Drugi format raportu

## Zadanie
Jeśli Twój system raportowy generuje PDF - dorób wariant HTML (albo odwrotnie).
Masz gotowe klocki: `tools/pdf_report.py`, `tools/html_report.py`, `tools/excel_report.py`.

## Kroki
1. Dodaj `report_writer`-owi drugie narzędzie raportowe.
2. Pozwól użytkownikowi wybrać format ("zrób raport jako stronę HTML").
3. Porównaj wyniki w katalogu `out/`.

## Czego się uczysz
Ten sam system, różne artefakty wyjściowe - elastyczność narzędziowa agenta.
