# Rozwiązanie: ex_25_tests (ćwiczenie NIEKODOWE + kod - testy, moduł 12)
...
- `test_sql_agent.py` - GOTOWY przykład. Odpala `AgentEvaluator.evaluate(agent_module=
  "ex_14_text_to_sql", eval_dataset=ex_17_eval_uruchom/sql_agent.evalset.json)`. `AgentEvaluator`
  sam dobiera `ex_17_eval_uruchom/test_config.json` (kryteria łagodne: trajektoria 0.0, response 0.5).
- `test_report_system.py` - ĆWICZENIE. Test jest, ale `@pytest.mark.skipif` pomija go,
  dopóki nie ma pliku `ex_17_eval_uruchom/report_system.evalset.json`.
...
2. **test_report_system**: nagraj test set systemu wieloagentowego i zapisz jako
   `ex_17_eval_uruchom/report_system.evalset.json`. Wtedy `skipif` przepuszcza test.
...
## Jak uruchomić
```bash
uv run pytest ex_25_tests                      # oba testy
uv run pytest ex_25_tests/test_sql_agent.py    # tylko SQL (przechodzi po rozwiązaniu ex_16)
```
...
Dlatego kryterium trajektorii trzymamy na `0.0` (z `ex_17_eval_uruchom/test_config.json`), a w
